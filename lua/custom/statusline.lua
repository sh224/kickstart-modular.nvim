...
vim.api.nvim_set_hl(0, 'StatusLineNC', { ctermfg = 8, ctermbg = 18 })
...
-- FIXME: Add working LSP status
M.get_active = function(self)
  local left = '%#StatusLineBufferNumberActive#' .. self:buffer_number() .. '%#StatusLinee#' .. self:filename() .. self:filetype()
  local right = self:command() .. self:git() .. self:encoding() .. self:file_format() .. self:row_and_column()
  return left .. '%=' .. right
end

M.get_inactive = function(self)
  return '%#StatusLineBufferNumber#' .. self:buffer_number() .. '%#StatusLinee#' .. self:filename()
end
